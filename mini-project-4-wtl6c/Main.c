...
void ButtonWork(void){
	...
	Button1RespTime = OS_MsTime() - Button1PushTime;							// Current OS Time - TimeStamp Button 1 Push = Response Time 
...
void SW1Push(void){
  if(OS_MsTime() > 20 ){ // debounce
    ...
    OS_ClearMsTime();  // at least 20ms between touches
		...
		Button1PushTime = OS_MsTime();												// TimeStamp Button 1 Push
  }
}
...
void Consumer(void){
	while(NumSamples < RUNLENGTH){
		...
		ConsumerCount++;																// total times the Consumer writes to the LCD ---- I didn't know if it should be += 4 
																										// Because technically it runs the LCD 4 times.
...
void Display(void){
	while(NumSamples < RUNLENGTH){
		...
		DisplayCount++;																			// total times the Display writes to the LCD
		
		BSP_LCD_Message(1,4,0,"PseudoCount: ",PseudoCount);
		OS_bSignal(&LCDFree);
	}
	
	// OS_Sleep(1);					Question 2   Part 1
	// OS_Suspend();				Question 3 	 Part 1
  OS_Kill();  // done
}
...
void Restart(void){
	...
	Button2RespTime = OS_MsTime() - Button2PushTime;							// Current OS Time - TimeStamp Button 2 Push = Response Time
...
void SW2Push(void){
  if(OS_MsTime() > 20 ){ // debounce
    ...
    OS_ClearMsTime();  // at least 20ms between touches
		Button2PushTime = OS_MsTime();				// TimeStamp Button 2 Push 
  }
}
