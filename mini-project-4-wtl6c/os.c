...
void OS_Wait(Sema4Type *semaPt){					
	...
	#ifdef blockSema									// Lecture 14 Slide 9
		OS_DisableInterrupts();
		(semaPt -> Value)--;
...
void OS_bWait(Sema4Type *semaPt){
	...
	#ifdef blockSema									// Read on Piazza that OS_bWait should have the same implementation as OS_Wait (Lecture 14 Slide 8) 
		OS_DisableInterrupts();
		(semaPt -> Value)--;
...
void OS_bSignal(Sema4Type *semaPt){
	...
	#ifdef blockSema												// Read on Piazza that OS_bSignal should have the same implementation as OS_Signal (Lecture 14 Slide 9)
...
void Scheduler(void){
	...
	#ifdef blockSema
		#ifdef prioritySched							// Lecture 17 Slide 7 
			uint32_t max = 255;
			tcbType * ptr;
			tcbType * bestPtr;
			RunPt = RunPt -> next;					// Added this, not in slides? :(
			ptr = RunPt;
	
			#ifdef aging										// Same as Lecture 17 Slide 7 but instead using the icnremented WorkPriority
				do{
					
					if((ptr -> WorkPriority < max) && (ptr -> blockPt == 0) && (ptr -> sleepCt == 0)) {
						max = ptr -> WorkPriority;
						bestPtr = ptr;
					}
				...
				RunPt = bestPtr;
				RunPt -> WorkPriority = RunPt -> FixedPriority;					// Reset Work Priority back to the Fixed Priority
				
			#else				// Fixed Priority
				do{
					
					if((ptr -> priority < max) && (ptr -> blockPt == 0) && (ptr -> sleepCt == 0)) {
						max = ptr -> priority;
						bestPtr = ptr;
					}
	...
	RunPt -> ExecCount = (RunPt -> ExecCount) + 1;	
}
...
void Timer2A_Handler(void){ 
	...
	MSTime++;
	...
	#ifdef aging													// Increase age as long as its not blocked and not sleeping
		...
		for(i = 0; i < NUMTHREADS; i++) {
			if(!(tcbs[i].available)) {
				...
				else if(tcbs[i].sleepCt) {
					
					tcbs[i].sleepCt -= 1;
				}
				
				if((tcbs[i].age > i) && (tcbs[i].WorkPriority > 0)) {  // pretty sure the i could be any value depending on the speed of aging you wanted.
					
					tcbs[i].age = 0;
					tcbs[i].WorkPriority -= 1;												// increase priority after age reaches certain threshold
				}
			}
		}
	#else
