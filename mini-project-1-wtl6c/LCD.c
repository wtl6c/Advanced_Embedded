...
void BSP_LCD_Message (int device, int line, int col, char *string, unsigned int value){
	...
	if(device) {
		BSP_LCD_DrawString(col, line, string, LCD_WHITE);							// Draws the X: and Y;
		BSP_LCD_DrawString(col + 3, line, val, LCD_WHITE);						// Displays the actuall X and Y coordinates
		
		if((value < 100) && (value > 10)) {
			BSP_LCD_DrawString(col + 5, line, 0, LCD_BLACK);						// If the value is between 100 and 10 makes sure their is no leading zero
		}
		
		else if((value < 10) && (value > 0)) {
			BSP_LCD_DrawString(col + 4, line, 0, LCD_BLACK);						// If the value is between 10 and 0 makes sure the two leading zeroes are gone
			BSP_LCD_DrawString(col + 5, line, 0, LCD_BLACK);
		}
	}
}
...
void BSP_LCD_DrawCrosshair(int16_t x, int16_t y, int16_t color) {
	...
	BSP_LCD_DrawFastVLine(x, (y - 3), 7, color);										// The minus 3 for Vertical and Horizontal allow for the cross hairs to cross
