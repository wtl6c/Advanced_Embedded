...
void OS_Wait(Sema4Type *semaPt){
	...
	start1 = StartCritical();
	
	while(semaPt -> Value <= 0) {
		EndCritical(start1);
		OS_Suspend();																								// Lecture 12 Slide 11 or Page 198 in the Book
		start1 = StartCritical();
	}
...
void OS_Signal(Sema4Type *semaPt){
	...
	start2 = StartCritical();
	semaPt -> Value += 1;																					// Lecture 12 Slide 11 or Page 198 in the Book
	EndCritical(start2);
}
...
void OS_InitSemaphore(Sema4Type *semaPt, long value){
	...
	start3 = StartCritical();
	semaPt -> Value = value;																			// Initializes semaPt value field to the passed in value
	EndCritical(start3);
}
...
void OS_Sleep(unsigned long sleepTime){
	...
	RunPt -> sleepCt = sleepTime;																// set the current thread to sleep by setting the sleepCt field to the passed in sleeptime
	OS_Suspend();
}
...
void Scheduler(void){
	...
	while((RunPt -> sleepCt) > 0) {														// As long as the sleepCt is greater than zero keep going to the next thread
...
void Timer2A_Handler(void){ 
	...
	while(x < NUMTHREADS) {																				// Loop through every thread
		if((tcbs[x].available == 0) && (tcbs[x].sleepCt > 0)) {			// If not available and has a sleep count greater than zero decrement the sleep count by 1
			tcbs[x].sleepCt -= 1;
		}
		x++;
	}
